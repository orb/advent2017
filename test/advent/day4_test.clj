...
(deftest test-parse-phrase
  (is (= ["a" "b" "c"] (parse-phrase "a b c"))))

(deftest test-valid-passphrase
  (is (valid-passphrase? ""))
  (is (valid-passphrase? "hello"))
  (is (valid-passphrase? "someone set up us the bomb"))
  (is (not (valid-passphrase? "i will not not do that")))
  (is (valid-passphrase? "aa bb cc dd aaa"))
  (is (not (valid-passphrase? "aa bb cc dd aa"))))

(deftest test-part1)
  (is (= 2 (part1 ["i am sam" "sam i am"])))
  (is (= 1 (part1 ["i do not like green eggs and ham" "i do not like them sam i am"])))
  (is (= 455 (part1 my-input)))


(deftest test-normalizer
  (is (= "a" (normalize "a")))
  (is (= "aa" (normalize "aa")))
  (is (= "aab" (normalize "baa")))
  (is (= "aab" (normalize "aba")))
  (is (= "biorrtu" (normalize "burrito"))))

(deftest test-valid-stronger
  (is (valid-stronger-passphrase? ""))
  (is (valid-stronger-passphrase? "hello"))
  (is (not (valid-stronger-passphrase? "hello hello")))
  (is (not (valid-stronger-passphrase? "hello ehllo"))))

(deftest test-part2
  (is (= 2 (part2 ["i am sam"
                   "sam i am"])))
  (is (= 1 (part2 ["i do not like green eggs and ham"
                   "i do not like them sam i am"])))
  (is (= 1 (part2 ["iiii oiii ooii oooi oooo"
                   "oiii ioii iioi iiio"])))
  (is (= 186 (part2 my-input))))
